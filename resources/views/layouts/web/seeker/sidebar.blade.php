...
<div class="col-lg-3 col-12 pb-4 min-vh-100" style="background-image: url({{ asset('images/circle/bg-login.svg') }});background-size: cover;background-position: center;">
    <div class="row">
        <!-- Nav -->
        <div class="col-12">
        ...
        @if(Auth::user()->hasRole(\App\Enum\RoleEnum::SEEKER))
            <div class="row text-center">
                <p class="text-white">- @if (isset(Auth::user()->about()->first()->job_title)) {{ Auth::user()->about()->first()->job_title }} @endif -</p>
            </div>
            <div class="row text-center">
                <button class="btn w-25 m-auto btn-sm btn-primary d-inline"><a href="{{ route('seeker.profile', Auth::id()) }}" class="text-light">Profile</a></button>
            </div>
        ...
        @elseif(Auth::user()->hasRole(\App\Enum\RoleEnum::COMPANY))
            <div class="row text-center">
                <p class="text-white">- @if (isset(Auth::user()->company->name)) {{ Auth::user()->company->name }} @endif -</p>
            </div>
            <div class="row text-center">
            ...
            <nav class="card bg-transparent px-3 py-4 mt-3 d-flex gap-3">
                ...
                <div class="@if(Route::currentRouteName() === 'company.about.index') bg-light rounded @endif">
                    <a href="{{ route('company.about.index') }}" class=" @if(Route::currentRouteName() === 'company.about.index') text-primary @else text-light @endif d-flex align-items-center d-inline-block ms-3 border-bottom p-2">
                        <i class="bi bi-building pe-2"></i>
                        <span class="fs-6 fw-normal">Company Information</span>
                    </a>
                </div>
                <div class="@if(Route::currentRouteName() === 'company.job.index') bg-light rounded @endif">
                    <a href="{{ route('company.job.index') }}" class=" @if(Route::currentRouteName() === 'company.job.index') text-primary @else text-light @endif d-flex align-items-center d-inline-block ms-3 border-bottom p-2">
                        <i class="bi bi-briefcase pe-2"></i>
                        <span class="fs-6 fw-normal">Jobs</span>
                    </a>
                </div>
